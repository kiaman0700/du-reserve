    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-lg font-bold text-slate-100">{adminUser.name}</h2>
              <span className="rounded bg-violet-500/15 px-2.5 py-0.5 text-xs font-bold text-violet-400 font-mono">
                도서관 관리실 (ADMIN)
              </span>
            </div>
            <p className="text-xs text-slate-400">운영자 ID: {adminUser.university_id}</p>
          </div>
        </div>

        <div className="text-xs text-slate-400 bg-slate-950/40 px-4 py-3 rounded-xl border border-slate-800 flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl text-center">
          ...
          <p className="text-2xl font-bold text-slate-200 mt-1 font-mono">{totalSeats}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl text-center">
          <p className="text-xs text-blue-500 font-medium">예약 가능</p>
          <p className="text-2xl font-bold text-blue-400 mt-1 font-mono">{availableSeats}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl text-center">
          <p className="text-xs text-slate-400 font-medium">이용 중</p>
          <p className="text-2xl font-bold text-slate-300 mt-1 font-mono">{occupiedSeats}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl text-center border-l-4 border-l-red-500">
          <p className="text-xs text-red-500 font-medium">검증 대기</p>
          <p className="text-2xl font-bold text-red-400 mt-1 font-mono animate-pulse">{pendingAuditSeats}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl text-center">
          <p className="text-xs text-violet-500 font-medium">수거/정리 중</p>
          <p className="text-2xl font-bold text-violet-400 mt-1 font-mono">{clearingSeats}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4 border-b border-slate-800 pb-3">
              ...
              <span className="text-[10px] bg-red-950 text-red-400 px-2 py-0.5 rounded border border-red-500/20 font-bold font-mono">
                FINAL AUDIT
              </span>
            </div>
            ...
            {auditList.length > 0 ? (
                ...
                {auditList.map(({ seat, report }, index) => {
                  ...
                  return (
                    <div key={seat.id} className={`pt-6 first:pt-0 space-y-4`}>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div>
                          <span className="text-sm font-bold text-slate-200">
                            Seat {String(seat.seat_number).padStart(2, "0")} ({seat.room_name})
                          </span>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between text-[11px]">
                            <span className="text-amber-400 font-semibold flex items-center gap-1">
                              <Camera className="h-3 w-3" /> 1차 신고 사진 (30분 전)
                            </span>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between text-[11px]">
                            <span className="text-red-400 font-semibold flex items-center gap-1">
                              <Camera className="h-3 w-3" /> 2차 신고 사진 (최종)
                            </span>
                            <span className="text-slate-500 font-mono">
                      ...
                      {/* Admin Decisions */}
                      <div className="flex flex-col sm:flex-row items-center gap-3 bg-slate-950/60 p-3 rounded-xl border border-slate-800/80 justify-between">
                        <div className="text-[11px] text-slate-400 text-center sm:text-left leading-normal max-w-md">
                          <span className="font-semibold text-slate-300">💡 이원화 개방 선택 가이드:</span><br />
                          현장에 개인 물품이 방치되어 있어 수거 작업이 필요하다면 <strong className="text-violet-400">지연 자동 개방</strong>을, 
                          자리가 이미 깨끗하거나 즉각 이용 가능하게 비우려면 <strong className="text-blue-400">즉시 수동 개방</strong>을 선택하세요.
                        </div>
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                          ...
                          <button
                            ...
                            className="flex-1 sm:flex-none flex items-center justify-center gap-1 bg-violet-600 hover:bg-violet-500 text-slate-100 px-3 py-2 rounded-lg font-bold text-xs transition-all hover:scale-[1.02] active:scale-[0.98]"
                          ...
                          <button
                            ...
                            className="flex-1 sm:flex-none flex items-center justify-center gap-1 bg-blue-600 hover:bg-blue-500 text-slate-100 px-3 py-2 rounded-lg font-bold text-xs transition-all hover:scale-[1.02] active:scale-[0.98]"
              ...
              <div className="py-12 text-center border border-dashed border-slate-800 rounded-xl">
                <CheckCircle className="h-10 w-10 text-slate-700 mx-auto mb-2" />
                <p className="text-sm font-semibold text-slate-400">현재 대기 중인 2차 신고 검증 건이 없습니다.</p>
                <p className="text-xs text-slate-500 mt-1">학생들이 2차 신고를 접수하면 타임라인에 실시간으로 반영됩니다.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <h3 className="text-sm font-bold text-slate-200 mb-3 uppercase tracking-wider">관제 좌석 요약도</h3>
            <div className="grid grid-cols-4 gap-2">
              {seats.map(seat => {
                let colorClass = "bg-slate-800 border-slate-700 text-slate-400";
                if (seat.status === "AVAILABLE") colorClass = "bg-blue-950/20 border-blue-500/30 text-blue-400";
                if (seat.status === "OCCUPIED") colorClass = "bg-slate-800/80 border-slate-700 text-slate-300";
                if (seat.status === "REPORTED_1ST") colorClass = "bg-amber-500/10 border-amber-500/50 text-amber-500 animate-pulse";
                if (seat.status === "REPORTED_2ND") colorClass = "bg-red-500/10 border-red-500/50 text-red-500 animate-pulse";
                if (seat.status === "CLEARING") colorClass = "bg-violet-500/10 border-violet-500/50 text-violet-400 animate-pulse";
